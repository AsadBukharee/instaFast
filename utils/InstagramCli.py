...
class InstagramCLI():
    def __init__(self, username=None, password=None):
        try:
            ...
            response = self.session.post('https://www.instagram.com/accounts/login/ajax/', data=self.credentials,
    ...
    def save_json(self, optimized, data):
        try:
            ...
            path = os.path.join(os.getcwd(), optimized['folder'], f"{optimized['sub_folder']}{dt_string}",
                                f"{optimized['filename']}{dt_file}.json")
    ...
    def search(self, query=None, save=True, optimizations={}):
        try:
            if query != None:
                ...
                res = self.session.get("https://i.instagram.com/api/v1/web/search/topsearch/", cookies=self.cookies,
                                       params=payload)
                if res.status_code == 200:
                    optimized = {
                    ...
                    for j, i in enumerate(res['users']):
                        ...
                        if optimized['save_hd_photo']:
                            ...
                            self.download(user_info['data']['user']['profile_pic_url_hd'], optimized,
                                          f"{i['user']['username']}_hd", "png")
    ...
    def abstraction(self, response, optimized, single_post):
        try:
            self.counter += 1
            post_id = response['id']
            if response['media_type'] == 1 and (
                    optimized['media_type'] == "photo" or optimized['media_type'] == "both"):
                ...
                if not optimized['raw_response']:
                    ...
                    self.posts['image'].append(
                        {**single_post,
                         **{"post_id": post_id, "image_url": img_url, "acccesibility_caption": accessibility_caption}})

            elif response['media_type'] == 2 and (
                    optimized['media_type'] == "video" or optimized['media_type'] == "both"):

                thumbnail = response['image_versions2']['candidates'][0]['url']
                video_url = response['video_versions'][0]['url']
                single_post['music_id'] = single_post['music_title'] = single_post['music_url'] = single_post[
                    'artist_id'] = single_post[
                    'artist_username'] = single_post['artist_name'] = single_post['is_private'] = single_post[
                    'is_verified'] = None
                ...
                if optimized['context'] == "reels":
                    if response['clips_metadata']['music_info'] != None:
                        ...
                        if not optimized['raw_response']:
                            ...
                            single_post['music_url'] = music_url['music_asset_info'][
                            ...
                            try:
                                ...
                                single_post['artist_username'] = music_url['music_consumption_info']['ig_artist'][
                                    'username']
                                single_post['artist_name'] = music_url['music_consumption_info']['ig_artist'][
                                    'full_name']
                                single_post['is_private'] = music_url['music_consumption_info']['ig_artist'][
                                    'is_private']
                                single_post['is_verified'] = music_url['music_consumption_info']['ig_artist'][
                                    'is_verified']
                            except:
                        ...
                        if optimized['save_music']:
                            self.download(
                                music_url['music_asset_info']['fast_start_progressive_download_url'], optimized,
                                music_name, 'mp3')
                ...
                if status:
                    ...
                    try:
                        while more_available and self.counter < optimized['count']:
                            ...
                            res = self.session.get(
                                f"https://i.instagram.com/api/v1/feed/user/{user_id}/", cookies=self.cookies,
                                params=payload).json()
                            more_available = res['more_available']

                            if optimized['raw_response'] and optimized['media_type'] == "both" and optimized[
                                'save_photo'] == False and optimized['save_video'] == False and optimized[
                ...
                if status:
                    ...
                    try:
                        while more_available and self.counter < optimized['count']:
                            ...
                            if optimized['raw_response'] and optimized['save_video'] == False and optimized[
                                'save_video_thumbnail'] == False and optimized['save_music'] == False:
                                self.posts.extend(res['items'])
                            elif optimized['raw_response']:
                                self.posts.extend(res['items'])
                                self.extract_media(
            ...
            for i in comments:
                ...
                if optimizations['child_comments'] and i.get('child_comment_count', 0) > 0:
                    ...
                    response = self.session.get(
                        f"https://i.instagram.com/api/v1/media/{optimizations['post_id']}/comments/{i['pk']}/child_comments/",
            ...
            if shortcode != None:
                ...
                try:
                    while more_available and self.counter < optimized['count']:
                        ...
                        res = self.session.get(
                            f"https://i.instagram.com/api/v1/media/{post_id}/comments/", cookies=self.cookies,
                            params=payload).json()
        ...
        try:
            if username != None:
                ...
                if status:
                    ...
                    if res.status_code == 200:
                        res = res.json()
                        lists = [i['node']['id'] for i in res['data']['user']['edge_highlight_reels']['edges']][
                                :optimized['highlight_count']]
                        batch = 3
                        for travel in range(0, len(lists), batch):
                            res = "&".join([f"reel_ids=highlight%3A{i}" for i in lists[travel:travel + batch]])
                            res = self.session.get(f"https://i.instagram.com/api/v1/feed/reels_media/?{res}",
                                                   cookies=self.cookies)
                            if res.status_code == 200:
                                res = res.json()
                                if optimized['raw_response'] and optimized['media_type'] == "both" and optimized[
                                    ...
                                    for i in res['reels_media']:
                                        items = i['items']
                                        max_len = len(items) if optimized['story_count'] >= len(items) else optimized[
                                            'story_count']
